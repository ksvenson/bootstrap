...
LEGEND_OPTIONS = {'bbox_to_anchor': (0.5, 0), 'loc': 'upper center', 'ncol': 2}
DISP_DECI = 5
...
def get_num_bins(x):
    ...
    return round(np.nanmean(r / width))


def title_helper(sample_dist, real_dist):
    return (r'$Y \sim \mathcal{N}$'
            + rf'$(\mu={sample_dist.mean()}, \sigma^2={sample_dist.var()})$, '
            + r'$X \sim \mathcal{N}$'
...
def bootstrap_sample_weights(raw_sample, weights, n_trials, moment, bs_prob=None, estimator=VAR_ESTIMATORS[0]):
    """
    Draw `len(raw_sample)` elements randomly from `raw_sample` according to probability
    distribution `bs_weights`. Then, estimate the `moment` (1 for mean, 2 for variance) using `weights` and `estimator`.
    ...
    if bs_prob is None:
        ...
        if moment == 1:
            return w_avg(bs_samples, bs_weights, axis=-1, estimator=estimator)
        elif moment == 2:
        ...
        if moment == 1:
            return np.mean(bs_samples, axis=-1)
        elif moment == 2:
...
if __name__ == '__main__':
    ...
    for diff_idx in (3,):
        ...
        raw_sample, weights = get_sample_weights(sample_dist, real_dist, n)
        print('-'*100)
        print(w_avg(raw_sample, weights, estimator=0))
        for i in AVG_ESTIMATORS:
            uniform = bootstrap_sample_weights(raw_sample, weights, n_trials, 1, bs_prob=None, estimator=i)
            weighted = bootstrap_sample_weights(raw_sample, weights, n_trials, 1, bs_prob=weights/np.sum(weights), estimator=i)
            data = np.stack((avg_estimates[i], uniform, weighted))
        ...
        for i in VAR_ESTIMATORS:
            uniform = bootstrap_sample_weights(raw_sample, weights, n_trials, 2, bs_prob=None, estimator=i)
            weighted = bootstrap_sample_weights(raw_sample, weights, n_trials, 2, bs_prob=weights/np.sum(weights), estimator=i)
